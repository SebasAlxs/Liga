<template>
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-4xl font-extrabold tracking-tight text-content mb-2">
          Sedes y <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">Escenarios</span>
        </h1>
        <p class="text-content-muted">Gestiona las canchas y complejos deportivos donde se juegan los partidos.</p>
      </div>
    ...
    <!-- Empty State -->
    <div v-else-if="!headquartersStore.headquarters.length" class="glass-card p-12 text-center rounded-3xl border border-border/5 max-w-lg mx-auto">
      <Icon name="lucide:map-pin" class="w-16 h-16 text-content-muted mx-auto mb-4" />
      <h3 class="text-xl font-semibold text-content">No hay sedes registradas</h3>
      <p class="text-content-muted mt-2">Registra la primera sede para poder programar partidos.</p>
    </div>
    ...
    <div v-else class="glass-card rounded-3xl border border-border/5 overflow-hidden">
      <div class="px-6 py-4 border-b border-border/5 flex items-center justify-between">
        <span class="text-sm text-content-muted">
          <span class="text-content font-bold">{{ headquartersStore.headquarters.length }}</span> sede{{ headquartersStore.headquarters.length !== 1 ? 's' : '' }} registrada{{ headquartersStore.headquarters.length !== 1 ? 's' : '' }}
        </span>
      </div>

      <div class="divide-y divide-border/5">
        <div
          v-for="item in headquartersStore.headquarters"
          :key="item.id"
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-content-muted truncate">{{ [item.city, item.address].filter(Boolean).join(' · ') || 'Sin dirección' }}</p>
          </div>
